...
- **Duplicate handling:** If two files still have the same datetime, identity (or no identity), byte size, normalized camera suffix, and output extension, the first gets the plain name, duplicates get `_DUP000`, `_DUP001`… suffixes. No content hash is calculated.
